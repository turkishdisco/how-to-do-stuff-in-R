...
plot(m, which = 3) # scale-location (for heteroskedasticity) 
```
...
```{r}
#| eval: true
model = glm(Survived ~ Pclass, data = titanic::titanic_train, family = binomial(link = "logit"))
...
Linear regression can also handle curves. How to know when you need this? A residual plot might show lots of negative or positive values at the extreme ends. 
```{r}
# quadratic term (one 'hump')
model = lm(mpg ~ I(wt^2) , data = mtcars) 

# cubic term (two 'humps')
model = lm(mpg ~ I(wt^3) , data = mtcars) 
```
...
```{r}
emmeans::emmeans(fit, ~ barthtot | c161sex,
                 at = list(att8 = c(30,50,70)), type = "response")
```


Another option is to use `gtsummary`
```{r}

m = glm(Survived ~ Pclass + Sex + Age, data = titanic::titanic_train, family = binomial(link = "logit"))
...
vcov_cluster = sandwich::vcovCL(model, cluster = ~ group)
model_cluster_ame = margins::margins(model, vcov = vcov_cluster)
...
coef(summary(model)) # fixed
VarCorr(model) # random
...
hist(residuals(model)) # as recommended by Linck & Cunnings 2015
plot(resid(model))
...
# performance package
performance::model_performance(model) # shows AIC, R2, ICC, RMSE all together
performance::check_model(model) # lots of tests - can take time if large model
performance::check_collinearity(model) # examine VIF
performance::r2(model) # determines most appropriate r2 (inc. conditional/marginal)
performance::icc(model) # intraclass correlation (proportion of variance explained by grouping structure)
