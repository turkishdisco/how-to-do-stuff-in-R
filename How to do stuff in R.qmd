...
# Sample in tidyverse
sample_n(df, size = 60, replace = F) # random sample of 60 rows from df
...
table %>%
  gtsummary::as_flex_table() %>% 
    # flextable::save_as_html(path = "./figs/2024/desc_stats_t_wave_redacted.html")
    # flextable::save_as_docx(path = "./figs/2024/desc_stats_t.docx")
```
...
Headings and subheadings are specified by prefixing hashtags - `#` for level 1, `##` for level 2, and so on. To cross-reference another section in your Quarto document (requires `number_sections` to be true in the YAML), add a label for each section in curly brackets:
