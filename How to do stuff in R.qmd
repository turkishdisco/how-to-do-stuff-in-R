...
If you need to compare column names across different dataframes, `janitor::compare_df_names(df1, df2, df3)` will give you a list of all the column names and their variable types (or you can just request the mismatches). Alternatively, `janitor::compare_df_cols_same(d1, d2, d3)` will just show you cases where column types differ (not whether columns are missing). 

```{r}
janitor::compare_df_names(df1, df2, df3,
...
# Selecting
Use `dplyr::select()` to select columns by name, index, or type. 
```{r}
