...
A left join will retain all rows in the first table (`df` below). 
```{r}
...
`str_trunc` - with and without ellipsis
```{r}
#| eval: true
long_string = "The Sleepwalkers: How Europe Went to War in 1914"
str_trunc(long_string, 16, ellipsis = "")
str_trunc(long_string, 19) # default ellipsis is ...
```
...
```{r}
# can be used as part of a case_when, e.g. check that the first element of TotalMarkA is NOT "E" or "%"
case_when(
...
# optional: pipe the above into this to remove percents from total columns/row
# %>% mutate(across(-c(cut, Total), ~ case_when(
#     cut == "Total" ~ as.character(parse_number(.)),
#     TRUE ~.))) %>% 
